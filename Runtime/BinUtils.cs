...
// This code was part of ZeroFormatter but modified for my needs
...
namespace SiegeUp.Core
{
    public static class BinaryUtil
    {
        ...
        public static unsafe double ReadDouble(ref byte[] bytes, int offset)
        {
            ...
            uint num = (uint)(bytes[offset] | bytes[offset + 1] << 8 | bytes[offset + 2] << 16 | bytes[offset + 3] << 24);
            ulong num2 = (ulong)(bytes[offset + 4] | bytes[offset + 5] << 8 | bytes[offset + 6] << 16 | bytes[offset + 7] << 24) << 32 | num;
            return *(double*)(&num2);
        }
        ...
        {
            var ensureSize = System.Text.Encoding.UTF8.GetMaxByteCount(value.Length);
            EnsureCapacity(ref bytes, offset, ensureSize);

            return System.Text.Encoding.UTF8.GetBytes(value, 0, value.Length, bytes, offset);
        }
